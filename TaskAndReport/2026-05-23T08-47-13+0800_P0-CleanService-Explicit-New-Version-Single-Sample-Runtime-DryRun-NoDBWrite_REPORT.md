...
Luceon acceptance used merge-base / three-dot branch evidence against current
`origin/main`. A two-dot diff against current `origin/main` includes unrelated
workflow-doc commits that landed on main after the Lucode runtime branch, so it
is not the acceptance diff.
