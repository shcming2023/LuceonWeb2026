# Task: P0 Controlled Live Task Progress Semantics Validation With Prod Testpdf Source
...
Task 85 was blocked because the previously documented sample directory did not exist locally. The user then corrected the sample source to:

`/Users/concm/prod_workspace/Luceon2026/testpdf`

Director confirmed the directory exists and contains multiple PDF candidates. This task reruns the same one-upload validation with the corrected sample source.

## Objective

Perform exactly one controlled small/medium sample upload in production, then observe whether the task page/API surfaces live MinerU progress semantics and a clear terminal or bounded ongoing state.

This remains a validation task, not a release-readiness task.
...
- Do not change code, production config, models, secrets, DB schema, MinIO objects, Docker volumes, or sample files.
...
- Prefer a small/medium PDF likely to parse quickly and emit MinerU progress logs.
- Record original absolute path, file size, and hash.
- Do not move, rename, edit, normalize, delete, or copy sample files into the repository.
- Do not use any other sample source.

If no suitable sample is safely identifiable within the observation time, stop and write a blocked report.
...
- Do not delete, prune, recreate, or mutate DB, MinIO, Docker volumes, task rows, artifacts, logs, or sample files.
...
- Write the completion report into `TaskAndReport/` using this naming rule:
  `2026-05-13T13-41-16+0800_P0-Controlled-Live-Task-Progress-Semantics-Validation-With-Prod-Testpdf-Source_REPORT.md`
