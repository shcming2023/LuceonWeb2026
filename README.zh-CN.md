...
LuceonWeb2026 是 Luceon 的 PDF -> MinerU -> MinerU-Popo -> ElegantBook 材料审查与交付工作台。本项目基于 MinerU Web 改造，保留文件上传、解析、原文件预览、PDF 溯源、材料元数据、Popo 资产盘点和 PDF/ElegantBook 比对能力。Raw/Clean/Standard 研究产物仍作为后台证据保留，但当前主 UI 已切换到源 PDF 与 ElegantBook 编译 PDF 的比对工作流。
...
- PDF 原文阅读和 PDF/ElegantBook 左右比对
- 材料元数据编辑、AI 辅助提取和目录筛选
- 保留原始 Markdown、按页 Markdown、Popo 增强 Markdown 和 OCR/Popo 对照基础能力
- 文件列表展示 MinerU task 状态、进度、task id 和耗时
- 支持 ElegantBook ZIP、编译 PDF、编译报告、渲染审查和终审报告等产物下载
...
<div align="center">
  ...
  <p>当前主审查入口 - <code>/review/compare</code> 中的源 PDF 与 ElegantBook 编译 PDF 比对</p>
