...
- 维护和修订 PRD
- 指挥 luplan 维护当前唯一有效 PRD，并验收 PRD 变更是否准确区分确定需求、调试策略和历史记录
- 拉取和部署代码
- 执行 smoke、UAT、专项验证
- 输出基线报告、验收报告、复盘文档
- 给 lucode 下达小批量任务书
...
- 在 Lucia 指挥下维护当前唯一有效 PRD
- 将已验证的项目事实、稳定需求、数据契约、状态机和验收标准及时写入 PRD
- 将调试参数、实验策略和压测观察记录为待验证策略，不得冒充确定需求
- 每次维护后主动汇报修改章节、确定需求、待验证策略、影响范围和下一步对 lucode 的约束
- 遵守 [luplan PRD 维护规程](/Users/concm/prod_workspace/Luceon2026/docs/prd/luplan-prd-maintenance.md)
...
2. [PRD 目录与唯一性索引](/Users/concm/prod_workspace/Luceon2026/docs/prd/README.md)
3. [luplan PRD 维护规程](/Users/concm/prod_workspace/Luceon2026/docs/prd/luplan-prd-maintenance.md)
4. [说明文档.md](/Users/concm/prod_workspace/Luceon2026/说明文档.md)
5. 本文：[Lucia-接班手册.md](/Users/concm/prod_workspace/Luceon2026/docs/reviews/Lucia-接班手册.md)
6. [UAT 测试指南](../../uat/README.md)
7. [一致性清理操作说明.md](/Users/concm/prod_workspace/Luceon2026/docs/reviews/一致性清理操作说明.md)
8. [任务状态诊断说明.md](/Users/concm/prod_workspace/Luceon2026/docs/reviews/任务状态诊断说明.md)

## 5. 当前已确认的稳定基线

当前最新已复核代码提交（Lucia 最新复核后）：

- Git commit：`773d328`
- 提交说明：`fix(p0): support string material ids in asset detail UAT`
...
- `pages-smoke.spec.ts + cross-page-consistency.spec.ts` 合并运行 10/10 通过
- `/__proxy/upload/audit/consistency` 返回 `ok: true`
- P0 Patch 2 已完成复核并补齐收口：字符串型 `materialId` 的资产详情页可打开，跨页面一致性 UAT 可运行闭环
- 继续暂停泛化开发
- 不进入 Wave3

P0 状态与追踪模型收口已确认完成的部分：

- 前端直接 `POST /__proxy/db/tasks` 创建占位任务的绕路逻辑已移除
- `AssetDetailPage` 已从 `if (!detail)` 改为 `if (!material && !detail)`
- `AssetDetailPage` 支持字符串型 `materialId`，不再只按数字 ID 查找 Material
- `cross-page-consistency.spec.ts` 通过 `/__proxy/upload/tasks` 直接上传真实 PDF 创建任务
- `cross-page-consistency.spec.ts` 会断言 API 响应体，避免拿 `undefined` 继续跑页面断言
- `pages-smoke.spec.ts` 真实资产详情断言已适配历史 `Material / assetDetails` 标题不一致

当前仍需注意：

- UAT 会持续生成测试素材、任务、MinIO 对象，`/audit/consistency` findings 数量可能随测试增长
- 当前 dry-run findings 多为历史脏数据与测试残留，不代表本轮主链路不可用
...
当前不是自由派单阶段。下一位 Lucia 接手后，应从最新 `main` 复验开始，而不是重新进入 Wave。

代班 Lucia 可以完整接替 Lucia 的职责，包括继续给 lucode 下达必要任务；但任务必须沿着当前 P0/P1 收敛计划延续，不能绕过当前阻塞项另起 Wave、泛化开发或中风险产品扩展。

执行边界：

- 若最新基线复验失败，代班 Lucia 可以继续下达必要的 P0/P1 收敛补丁任务。
- 若最新基线复验通过，代班 Lucia 应输出复验结论，并围绕生产交付目标处理下一项生产准入阻塞项。
- 任何新任务仍必须小批量、低风险、可验收，并要求 lucode 完成后主动同步 GitHub。
- 禁止跳过基线复验直接进入 Wave3 或泛化开发。
