#!/usr/bin/env python3
"""Backfill exact same-unit Raw context-window bboxes for manual review."""
...
SOURCE_MATCH_RULE = "raw_context_window.compact_exact_same_unit_for_manual_review"
...
def exact_windows(clean_text: str, rows: list[dict[str, Any]], *, max_window: int = 4) -> list[dict[str, Any]]:
...
def build_or_apply(standard_dir: Path, *, apply: bool) -> dict[str, Any]:
    ...
    for item in remaining.get("items") or []:
        ...
        if len(key) < 24:
            continue
        heading_path = item.get("heading_path") if isinstance(item.get("heading_path"), list) else []
        leaf = str(heading_path[-1]) if heading_path else ""
        groups[(leaf, key)].append(item)

    candidates: list[dict[str, Any]] = []
    for (leaf, key), items in sorted(groups.items()):
        items.sort(key=lambda item: block_sort_key(str(item.get("block_id") or "")))
        rows = same_unit_rows(raw_rows, items[0].get("heading_path") or [])
        matches = exact_windows(str(items[0].get("clean_text_preview") or ""), rows)
        group_record = {
            "heading": leaf,
            "compact_key_preview": key[:120],
            "outcome_count": len(items),
            "candidate_count": len(matches),
            "outcome_block_ids": [item.get("block_id") for item in items],
            "candidate_source_orders": [match.get("source_orders") for match in matches],
        }
        if len(matches) != len(items):
            group_record["status"] = "not_exact_deterministic"
    ...
    for candidate in candidates:
        ...
        if not packet or not outcome:
            ...
            if apply:
                ...
                outcome["reviewer"] = "system:raw_context_exact_bbox_backfill"
                outcome["reviewed_at"] = now
                outcome["notes"] = (
                    "Source page/bbox backfilled from exact same-unit Raw context-window evidence; "
    ...
    report = {
        "schema": "luceon-standard-raw-context-exact-bbox-backfill/v1",
        "standard_dir": str(standard_dir),
        "mode": "apply" if apply else "audit",
        "policy": "raw_context_compact_exact_same_unit_backfill_is_source_location_evidence_not_closure",
    ...
    write_json(standard_dir / "raw_context_exact_bbox_backfill_report.json", report)
    return report
