#!/usr/bin/env python3
"""Close Math formula outcomes from a remaining exact-surface risk audit."""
...
def is_closable(risk_item: dict[str, Any]) -> tuple[bool, str]:
    ...
    return True, "remaining_raw_assignment_exact_surface_safe"


def close_from_audit(standard_dir: Path, risk_audit_path: Path, *, apply: bool) -> dict[str, Any]:
            ...
            skipped.append(
                {
                    ...
                    "formula_bucket": risk_item.get("formula_bucket"),
            ...
            item.update(
                {
                    ...
                    "reviewer": "system:math_remaining_exact_surface_safe_closure",
                    "reviewed_at": now,
                    "notes": "Closed by remaining raw_assignment.located_exact surface-safe audit; containment and digit-spacing excluded.",
                    "evidence": [
    ...
    if apply:
        ...
        outcomes["math_remaining_exact_surface_safe_closure_summary"] = {
...
def main() -> int:
    args = parse_args()
    result = close_from_audit(args.standard_dir, args.risk_audit, apply=args.apply)
    report = {
        "schema": "luceon-standard-math-remaining-exact-surface-safe-closure/v1",
        "standard_dir": str(args.standard_dir),
        "risk_audit": str(args.risk_audit),
        "mode": result["mode"],
        "policy": "close_only_remaining_raw_assignment_exact_surface_safe_digit_spacing_and_containment_excluded",
    ...
    write_json(args.standard_dir / "math_remaining_exact_surface_safe_closure_report.json", report)
    if args.apply:
        ...
        closure["math_remaining_exact_surface_safe_closure_report"] = "math_remaining_exact_surface_safe_closure_report.json"
