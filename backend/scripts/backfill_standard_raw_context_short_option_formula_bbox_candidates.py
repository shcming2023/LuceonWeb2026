#!/usr/bin/env python3
"""Backfill short option-formula Raw context bboxes for manual review."""
...
SOURCE_MATCH_RULE = "raw_context_window.short_option_formula_exact_same_unit_for_manual_review"
...
def is_short_option_formula(clean_text: str) -> bool:
    stripped = clean_text.strip()
    if not re.match(r"^[A-Z]\s*\$?\$?", stripped):
        return False
    if not any(token in clean_text for token in ("=", "\\frac", "+", "-")):
        return False
    key = compact_key(clean_text)
    return 8 <= len(key) < 24


def unique_option_formula_match(clean_text: str, rows: list[dict[str, Any]]) -> dict[str, Any] | None:
    key = compact_key(clean_text)
    ...
    for page_rows in by_page.values():
        ...
        for start in range(len(page_rows)):
            for size in range(1, 4):
                ...
                if len(window) != size:
                    continue
                if compact_key(" ".join(raw_row_text(row) for row in window)) == key:
                    matches.append(window)
    if len(matches) != 1:
        return None
    window = matches[0]
    return {
        "source_page_number": int(window[0]["page_idx"]) + 1,
        ...
        "source_content_preview": " ".join(raw_row_text(row) for row in window)[:500],
    }
...
def build_or_apply(standard_dir: Path, *, apply: bool) -> dict[str, Any]:
    ...
    for item in audit.get("items") or []:
        ...
        if not is_short_option_formula(clean_text):
            continue
        match = unique_option_formula_match(clean_text, same_unit_rows(raw_rows, item.get("heading_path") or []))
        if not match:
            continue
        candidates.append(
            {
    ...
    for candidate in candidates:
        ...
        if not packet or not outcome:
            ...
            if apply:
                ...
                outcome["reviewer"] = "system:raw_context_short_option_formula_bbox_backfill"
                outcome["reviewed_at"] = now
                outcome["notes"] = (
                    "Source page/bbox backfilled from unique short option-formula Raw context evidence; "
    ...
    report = {
        "schema": "luceon-standard-raw-context-short-option-formula-bbox-backfill/v1",
        "standard_dir": str(standard_dir),
        "mode": "apply" if apply else "audit",
        "policy": "raw_context_short_option_formula_exact_same_unit_backfill_is_source_location_evidence_not_closure",
    ...
    write_json(standard_dir / "raw_context_short_option_formula_bbox_backfill_report.json", report)
    return report
