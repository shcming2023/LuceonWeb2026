#!/usr/bin/env python3
"""Backfill same-unit Raw page containment bboxes for manual review."""
...
SOURCE_MATCH_RULE = "raw_context_window.compact_contained_same_unit_for_manual_review"
...
def block_sort_key(block_id: str) -> tuple[int, str]:
    match = re.search(r"(\d+)$", block_id)
    return (int(match.group(1)) if match else 10**12, block_id)


def containment_candidates(clean_text: str, rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    clean_key = compact_key(clean_text)
    if len(clean_key) < 24:
        return []
    ...
    for page_idx, page_rows in by_page.items():
        ...
        parts = [compact_key(raw_row_text(row)) for row in page_rows]
        joined = "".join(parts)
        start = joined.find(clean_key)
        if start < 0:
            continue
        end = start + len(clean_key)
        covering_rows: list[dict[str, Any]] = []
        pos = 0
        for row, part in zip(page_rows, parts):
            next_pos = pos + len(part)
            if next_pos > start and pos < end:
                covering_rows.append(row)
            pos = next_pos
        if not covering_rows:
            continue
        source_text = " ".join(raw_row_text(row) for row in covering_rows)
        candidates.append(
            {
                "source_page_number": page_idx + 1,
                "source_bbox": bbox_union(covering_rows),
                "window_size": len(covering_rows),
                "raw_indices": [row.get("_raw_index") for row in covering_rows],
                "source_orders": [row.get("source_order") for row in covering_rows if row.get("source_order") is not None],
                "source_unit_title": covering_rows[0].get("unit_title") or "",
                "containment_offset": start,
                "source_content_preview": source_text[:500],
            }
        )
    candidates.sort(key=lambda item: item.get("source_orders", [10**12])[0] if item.get("source_orders") else 10**12)
    return candidates
...
def build_or_apply(standard_dir: Path, *, apply: bool) -> dict[str, Any]:
    ...
    for item in sorted(remaining.get("items") or [], key=lambda row: block_sort_key(str(row.get("block_id") or ""))):
        ...
        matches = containment_candidates(clean_text, rows)
        record = {
        ...
        if len(matches) != 1:
            record["status"] = "not_unique_containment_candidate"
    ...
    for candidate in candidates:
        ...
        if not packet or not outcome:
            ...
            if apply:
                ...
                packet["source_raw_type"] = "context_window_containment"
                ...
                packet["source_context_window"] = {
                    ...
                    "containment_offset": candidate.get("containment_offset"),
                }
                ...
                outcome["reviewer"] = "system:raw_context_containment_bbox_backfill"
                outcome["reviewed_at"] = now
                outcome["notes"] = (
                    "Source page/bbox backfilled from unique same-unit Raw page compact containment; "
                    "source crop and manual visual review are still required."
                )
                outcome["evidence"] = [
                    "remaining_bbox_blocker_audit.json",
                    SOURCE_MATCH_RULE,
    ...
    report = {
        "schema": "luceon-standard-raw-context-containment-bbox-backfill/v1",
        "standard_dir": str(standard_dir),
        "mode": "apply" if apply else "audit",
        "policy": "unique_same_unit_page_compact_containment_backfill_is_source_location_evidence_not_closure",
    ...
    write_json(standard_dir / "raw_context_containment_bbox_backfill_report.json", report)
    return report
...
def main() -> int:
    ...
    print(json.dumps({key: value for key, value in report.items() if key not in {"items", "skipped_sample"}}, ensure_ascii=False))
    return 0
