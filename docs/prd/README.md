...
- 当前维护状态：2026-05-25 起采用 Luceon 单负责人收口流程；Luceon 统一负责规划、需求、架构、产品、代码、测试、验收和最终文档落账。历史 Lucode 流程仅作为证据记录保留。
...
3. 新需求进入实现前，必须由用户或当前 Luceon 单负责人流程判断其属于：
   - 已确定需求：写入当前有效 PRD 的主体章节或验收标准。
   - 调试策略：写入 PRD 的策略/风险/待验证区，不得冒充稳定需求。
   - 历史记录：写入 PRD 变更记录或相关专项报告。
4. 如未来需要升级 PRD 版本，必须先明确宣布旧版废止、新版成为唯一有效 PRD，并同步更新本索引。

## 6.9.1 后协同状态

历史 Lucia/Lucode 以及后续多角色团队协同均已归档。当前活跃流程以 `docs/codex/roles/luceon.md` 和 `TaskAndReport/README.md` 为准；本地私有 `AGENTS.md` 只作为运行提示，不作为 GitHub 项目事实源。
