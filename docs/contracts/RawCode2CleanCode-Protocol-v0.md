...
其中，`full.md` 是全书级 Markdown 视图，`chapters/*/unit.md` 是给 LLM 清洗的主输入，必须是已经按上游目录结构切好的结构锁定 Markdown。`chapters/*/raw.md` 保留为兼容预览，不作为唯一真相。v0 的单章节 pilot 可以只要求一个章节目录存在，但顶层 `manifest.json` 与 `toc.json` 必须存在。
...
| `chapters/*/unit.md` | 必需 | 单章节/小节待清洗主输入 Markdown；保留上游目录标题、父路径和 unit 边界注释。 |
| `chapters/*/raw.md` | 兼容必需 | 人类可读预览和旧工具兼容输入；不得替代 `unit.md` 的结构锁定主输入职责。 |
...
`unit.md` 是节点 8 输出给节点 9 的低 token 主输入。它从 JSON pack 解开，但必须保持上游 Luceon canonical TOC 已确定的结构边界。LLM 读取 `unit.md` 做清洗，程序读取 `source_map.json`、`image_map.json` 和 `chunk_manifest.json` 做审计验证。

`unit.md` 推荐包含轻量边界注释：

```md
<!-- luceon:unit_id=section_4_1 -->
<!-- luceon:parent_path=Unit 1 > 4 Collecting, organising and displaying data -->
<!-- luceon:pack_level=3 -->
<!-- luceon:boundary_basis=structure-level -->
<!-- luceon:source_blocks=2928,2929,2930 -->
<!-- luceon:asset_hashes=895209977cd9d86867c07fcfc6f709604e029c454984782838f2809ad876be0.jpg -->
...
节点 9 不得用 `unit.md` 重新判断全书目录结构；它只能在该 unit 内清洗文字、公式、题目和图片引用。
  ...
  "chapters": [
    {
      ...
      "title": "Chapter 1",
      "path": "chapters/chapter_001/unit.md",
      "raw_preview_path": "chapters/chapter_001/raw.md"
    }
  ]
}
```
  ...
  "nodes": [
    {
      ...
      "title": "Chapter 1",
      "order": 1,
      "unit_path": "chapters/chapter_001/unit.md",
...
```json
{
  ...
  "status": "PASS",
  "input": {
    "input_markdown": "rawcode/sample-material/v0/chapters/chapter_001/unit.md",
    "input_markdown_role": "structure_locked_unit_markdown",
    "unit_md": "rawcode/sample-material/v0/chapters/chapter_001/unit.md",
...
| Unit Markdown Builder | `cleaning_unit_pack`、`toc.json`、Popo/Luceon 结构 | `chapters/*/unit.md` + sidecar JSON | 依据已重建目录切块，并记录边界；不让 LLM 决定目录。 |
| Rule PreCleaner | `unit.md` | preclean Markdown | 只删除高置信噪音，不改变 unit 边界。 |
| LLM Cleaner | preclean `unit.md`、极简目录上下文、图片清单 | 结构化清洗结果 | 忠实整理，不允许创造内容，不允许生成或改变目录结构。 |
...
Prompt 中必须明确：LLM 只能清洗、纠错、整理和标准化当前结构锁定 `unit.md`；不得总结、扩写、替换教材表达、删除可能属于正文的内容；不得新增、删除、升降级或重命名全书目录结构；遇到不确定内容必须写入 `unresolved_items`。

### 8.1 P0.1 受控 LLM 接口

P0.1 的核心变化是把原先的单体确定性 cleaner 拆成 **Rule PreCleaner → LLM Cleaner → Rule PostProcessor → Validator** 四段式流程。LLM Cleaner 只允许处理已经由 Rule PreCleaner 去除高置信版面噪音后的单章节/小节 `unit.md`，并且必须返回第 8 节定义的结构化 JSON。程序不得直接把 LLM 返回文本当作最终 `clean.md`，而必须经过 JSON 校验、图片引用校验、后处理和本地验证后才能落盘。
...
| 输入内容 | `preclean_markdown`、`input_markdown_role`、`chapter_context`、`structure_lock`、`image_map`、`source_map_summary`、`constraints`。 |
...
```json
{
  ...
  "constraints": [
    ...
    "Do not create, delete, promote, demote, or rename book-structure headings.",
...
| 成功标准 | 验收方式 |
| --- | --- |
| 能生成 RawCode 章节样本 | `rawcode/{materialId}/v0/chapters/chapter_001/unit.md` 存在，`raw.md` 作为兼容预览存在。 |
