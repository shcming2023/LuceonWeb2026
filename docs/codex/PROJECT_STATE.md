...
- Adaptive evidence-pack first-pass implementation is assigned to `TASK-20260508-145945-P0-Adaptive-Evidence-Pack-First-Pass-For-Large-AI-Metadata-Inputs`. Approved trigger thresholds are Markdown length greater than `50000`, source file size greater than `10000000` bytes, or parsed files count greater than `50`. Production runtime mutation and production release-readiness claims remain unauthorized.
...
| TD-017 | Open | Medium-large parsed documents can still enter AI metadata first pass through the legacy sampler with timeout-prone input size. | Task 30 diagnosis accepted. Task 31 is assigned to implement adaptive evidence-pack selection with strict no-skeleton semantics preserved; production runtime validation remains pending after code-level acceptance. |
