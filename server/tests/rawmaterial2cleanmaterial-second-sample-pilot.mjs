...
const SAMPLE = Object.freeze({
  materialId: '2241872074049025',
  taskId: 'task-1779085090677',
  title: '蓝月、血月、橙月？月亮为啥还会变色？',
  parsedZip: {
    bucket: 'eduassets-parsed',
    object: 'parsed/2241872074049025/mineru-result.zip',
  },
  rawObject: 'mineru/2241872074049025/v1/content_list_v2.json',
...
const realApply = process.env.RAW2CLEAN_SECOND_SAMPLE_REAL_APPLY === '1';
const nowValue = process.env.RAW2CLEAN_SECOND_SAMPLE_NOW || '2026-05-25T06:07:10.000Z';
const outDir = '/tmp/luceon-task285-raw2clean-second-sample';
...
function buildCleanOperatorDecision(cleanSummary, taskSummary) {
  return {
    ...
    decidedAt: nowValue,
    decidedBy: 'Luceon',
    reason: 'Second canonical sample pilot accepted CleanMaterial for raw2clean repeatability validation',
    artifactSnapshot: {
...
function buildAcceptedRaw2CleanSummary({ output, candidateRef, candidatePreview }) {
  ...
  const decision = {
    ...
    decidedAt: nowValue,
    decidedBy: 'Luceon',
    scope: 'second-sample-durable-repeatability-pilot',
    ...
    reason: 'Second canonical sample pilot durable accepted decision apply',
    boundaries: {
...
async function main() {
  ...
  const existingV1JobId = `luceon-${SAMPLE.taskId}-toc-rebuild-v1`;
  const existingV1Job = await queryCleanServiceJob(existingV1JobId);
  const cleanAssetVersion = existingV1Job?.status === 'failed' ? 'v2' : SAMPLE.cleanAssetVersion;
  const previousFailedCleanJob = existingV1Job?.status === 'failed'
    ? { status: 'failed', cleanState: 'failed', assetVersion: 'v1', jobId: existingV1JobId }
    : null;
  ...
  const taskForCleanService = {
    ...
    metadata: {
      ...(preTask.metadata || {}),
      ...(previousFailedCleanJob ? { cleanServiceJobs: { ...(preTask.metadata?.cleanServiceJobs || {}), 'toc-rebuild-failed-shadow': previousFailedCleanJob } } : {}),
      rawMaterial: {
        version: 'v1',
        mineru: {
          contentListV2: {
  ...
  assert.equal(request.job_id, `luceon-${SAMPLE.taskId}-toc-rebuild-${cleanAssetVersion}`);
  assert.equal(request.asset_version, cleanAssetVersion);
  assert.equal(request.inputs[0].source.object, SAMPLE.rawObject);
  const existingTargetJob = await queryCleanServiceJob(request.job_id);
  ...
  const deps = {
    ...
    applyCleanMetadataPersistencePlan: async ({ plan }) => {
      ...
      return {
        ...
        reason: 'captured clean metadata plan for combined second-sample apply',
      };
    },
    dbClient: {
  ...
  assert.equal(cleanResult.assetVersion, cleanAssetVersion);
  ...
  const draftResult = await buildRawMaterial2CleanMaterialArtifactBackedDraftDryRun({
    ...
    task: taskWithClean,
    currentAssetVersion: cleanAssetVersion,
  ...
  console.log(JSON.stringify({
    ...
    cleanService: {
      ...
      assetVersion: cleanAssetVersion,
