...
3. Set `MINIO_INTERNAL_ENDPOINT` for containers, `MINIO_PUBLIC_ENDPOINT` for browser downloads, and `LUCEON_EXTERNAL_BACKUP_PATH` to a writable external volume. The external backup path is a production preflight gate.
4. Run `./scripts/preflight`, `./scripts/pull-or-load`, `./scripts/migrate`, `./scripts/start`, and `./scripts/verify` in that order.
5. Start only one Workflow V2 worker against the production database. Stop the staging worker first if both environments share Workflow MySQL or MinIO.
