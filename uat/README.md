...
# 指定目标地址和公网主机名
BASE_URL=http://YOUR_HOST:8080 PUBLIC_HOST=YOUR_HOST npx pnpm@10.4.1 --dir uat exec playwright test
...
- [ ] 浏览器打开 `http://YOUR_HOST:8080`，自动跳转到 `/cms/tasks`
- [ ] 页面正常渲染，侧边栏导航可切换
...
BASE_URL=http://127.0.0.1:8080 npx pnpm@10.4.1 --dir uat exec playwright test tests/pipeline-consistency.spec.ts

# 执行页面可用性回归（防止 React 运行时崩溃）
BASE_URL=http://127.0.0.1:8080 npx pnpm@10.4.1 --dir uat exec playwright test tests/pages-smoke.spec.ts
